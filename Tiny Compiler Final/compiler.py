...
def g():
    if nxt is '>':
        scan()
        if nxt.islower():
            mips("# Read input as double into {0}($s1)\n".format(M[nxt]))
            mips("        li      $v0, 7\n")
            mips("        syscall\n")
            mips("        s.d     $f0, {0}($s1)\n\n".format(M[nxt]))
        else:
...
def scan():
    global src, nxt, line_count, char_count
    c = getc()
    while c.isspace():
        src += c
        if c is '\n':
...
def reset():
    ...
    # Compiler variables
    global line_count, char_count, accepted, rejected
    line_count = 1
    char_count = 0
    # accepted = 0
    # rejected = 0
    ...
    # Assembly variables
    global code, pow_func, tmp_count, while_count, if_count, include_pow, neg
    code = "\n"
    pow_func = ""
    tmp_count = 0
    while_count = 0
...
while True:
    ...
    if successful:
        ...
        fout.write(code)
        fout.write(FRAMEWORK_END.format(pow_func))
        fout.close()
    reset()
sys.stdout.write("{0} Accepted, {1} Rejected, {2} Total\n".format(accepted, rejected, accepted + rejected))
